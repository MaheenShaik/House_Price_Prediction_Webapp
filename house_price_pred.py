...
def new_model(input_data):
    
   
    #changing the input data to numpy array
    input_data_array=np.asarray(input_data)
    #input data reshaped as we are predicting for one instance
...
#creating a button for prediction
    if st.button('Predict House Price'):
        price_prediction=new_model([CRIM,ZN,INDUS,CHAS,NOX,RM,AGE,DIS,RAD,TAX,PTRATIO,B,LSTAT])
